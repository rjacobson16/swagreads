...
export const fetchEveryUserReviews = () => dispatch =>
  ApiUtil.fetchEveryUserReviews()
    // .then(reviews => {
    //   console.log("REVIEW DATA", reviews);
    //   return Promise.resolve(reviews);
    // })
    .then(reviews => dispatch(receiveEveryUserReviews(reviews)));
...
export const deleteReview = id => dispatch => ApiUtil.deleteReview(id).then();

const receiveEveryUserReviews = reviews => ({
  type: RECEIVE_EVERY_USER_REVIEWS,
  everyUserReviews: reviews
});
